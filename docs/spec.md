...
```
原價 1000、早鳥 -10%、團體 -10%、優惠碼 -100 元
應付 = round(1000 × 0.9 × 0.9) − 100 = 710
```

優惠碼是**現金券**,不被百分比稀釋。所有計算以整數分為單位,
每一層乘完就 `Math.round`,不留浮點。
...
**全部成功或全部失敗**(同一個 `db.batch()`)。團體折扣以 `seat_nos.length` 為依據。
