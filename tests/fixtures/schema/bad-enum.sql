...
CREATE TABLE seat_holds (id INTEGER PRIMARY KEY, event_id INTEGER NOT NULL, seat_no TEXT NOT NULL, member_id INTEGER NOT NULL, status TEXT NOT NULL CHECK (status IN ('holding','confirmed','expired','cancelled','refunded')), expires_at INTEGER NOT NULL, created_at INTEGER NOT NULL) STRICT;
